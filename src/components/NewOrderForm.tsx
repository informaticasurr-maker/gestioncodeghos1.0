...
export const NewOrderForm: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {formError && (
        <div className="p-4 bg-rose-50 border-2 border-rose-400 text-rose-800 rounded-xl text-sm font-semibold flex items-center justify-between shadow-sm">
      ...
      <form onSubmit={handleSubmitOrder} className="space-y-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-lg transition"
            >
                  ...
                  return (
                    <>
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-slate-900 text-sm">{sel.name}</span>
                        <span className="text-[11px] font-mono text-indigo-700 bg-white px-2 py-0.5 rounded border border-indigo-200 font-semibold">
                          {sel.phone}
                        </span>
                      </div>
                      <p className="text-slate-600">DNI / CUIT: {sel.documentId || 'No especificado'}</p>
                      <p className="text-slate-600">Email: {sel.email || 'No especificado'}</p>
                      <p className="text-slate-600">Dirección: {sel.address ? `${sel.address}, ${sel.city}` : 'No especificada'}</p>
                      {sel.notes && <p className="text-slate-500 italic">Nota: {sel.notes}</p>}
                    </>
                  );
                })()}
              </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
          <h3 className="font-bold text-sm text-slate-900 uppercase tracking-wider flex items-center gap-2 border-b border-slate-200 pb-2">
            <DeviceIcon type={deviceType} className="w-4 h-4 text-indigo-600" />
            <span>2. Dispositivo a Reparar</span>
          </h3>

          {/* Device Type Selector */}
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-2">Tipo de Equipo:</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Modelo Exacto *:</label>
              <input
                type="text"
                value={deviceModel}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">N° de Serie / IMEI:</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Color / Versión:</label>
              <input
                type="text"
                value={deviceColor}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            ...
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
              ...
              <input
                ...
                className="w-full text-[11px] p-1.5 mt-2 border border-slate-300 rounded bg-white"
              />
            </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                ...
                <button
                  ...
                  className="px-2.5 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold shrink-0"
                >
                  +
                </button>
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                ...
                <input
                  ...
                  className="w-12 text-xs p-1 text-center border border-slate-300 rounded-lg bg-white font-mono"
                  title="Cantidad"
                />
                <button
                  type="button"
                  ...
                  className="px-2.5 py-2 bg-teal-600 hover:bg-teal-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold shrink-0"
                >
                  +
                </button>
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                ...
                <input
                  ...
                  placeholder="$"
                  className="w-16 text-xs p-2 border border-slate-300 rounded-lg bg-white font-mono"
                />
                <button
                  type="button"
                  onClick={handleAddManualService}
                  disabled={!manualServiceName.trim()}
                  className="px-2.5 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-white rounded-lg text-xs font-semibold shrink-0"
                >
                  +
                </button>
              </div>
          ...
          {sparePartsList.length > 0 && (
            <div className="border border-teal-200 bg-teal-50/40 rounded-xl overflow-hidden p-3 space-y-2">
              <div className="flex items-center justify-between text-xs font-bold text-teal-900">
                <span className="flex items-center gap-1.5">
                  <Boxes className="w-4 h-4 text-teal-600" />
                  Repuestos Asignados ({sparePartsList.length})
                </span>
                <span className="font-mono">{formatMoney(sparePartsTotal)}</span>
              </div>
              <div className="divide-y divide-teal-100 bg-white rounded-lg border border-teal-200 text-xs">
                {sparePartsList.map((part) => (
                  <div key={part.id} className="p-2 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-900 font-mono">{formatMoney(part.subtotal)}</span>
                      <button
                        type="button"
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden">
                  ...
                  <tr key={item.id || idx}>
                    ...
                    <td className="p-2.5 text-right">
                      <input
                        ...
                        className="w-24 text-right p-1 text-xs border border-slate-300 rounded font-mono font-semibold"
                      />
                    </td>
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-200 flex flex-wrap justify-between items-center gap-4">
              <div className="flex items-center gap-2 text-xs">
                <span className="text-slate-500 font-medium">Garantía:</span>
                <div className="flex items-center gap-1">
                  <input
                    ...
                    className="w-16 p-1 text-xs border border-slate-300 rounded font-mono font-bold text-center bg-white"
                  />
                  <span className="font-semibold text-slate-800">días</span>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-slate-700">Total Presupuestado:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full text-xs p-2.5 border border-slate-300 rounded-lg bg-white leading-relaxed"
              />
            </div>
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div>
                <input
                  ...
                  className="w-full text-xs text-slate-500 file:mr-2 file:py-1 file:px-2.5 file:rounded file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
                />
              </div>
              ...
              <div>
                <button
                  ...
                  className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold transition"
                >
                  + Adjuntar Foto
                </button>
              </div>
            ...
            {photos.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2">
                {photos.map((p, idx) => (
                  <div key={p.id} className="relative group border border-slate-300 rounded-lg overflow-hidden bg-white p-1">
                    <img
                      src={p.url}
                      alt={p.description}
                    ...
                    />
                    <p className="text-[10px] text-slate-600 mt-1 font-medium truncate">{p.description}</p>
                    <button
                      type="button"
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            {/* Initial Status */}
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Estado de Ingreso:</label>
              <select
                value={initialStatus}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Fecha Estimada de Retiro:</label>
              <input
                type="date"
            ...
            {/* Seña / Deposit */}
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Seña Abonada (Opcional):</label>
              <div className="flex gap-1.5">
                <input
                  type="number"
                  min="0"
                ...
                {depositNum > 0 && (
                  <select
                    ...
                    className="text-xs p-1 border border-slate-300 rounded-lg bg-white"
                  >
          ...
          <div className="bg-slate-900 text-white p-4 rounded-xl flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-6">
              <div>
                ...
                <p className="text-xl font-bold font-mono text-white">{formatMoney(totalAmount)}</p>
              </div>
              <div>
                <span className="text-xs text-emerald-400">Seña Ingresada</span>
                <p className="text-xl font-bold font-mono text-emerald-400">{formatMoney(depositNum)}</p>
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setActiveTab('ordenes')}
                className="px-4 py-2.5 rounded-lg border border-slate-700 text-xs font-semibold text-slate-300 hover:text-white hover:bg-slate-800 transition"
              >
                Cancelar
              </button>
              ...
              <button
                ...
                className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-bold shadow-lg transition transform active:scale-95 flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-200">
            ...
            <form onSubmit={handleCreateClientSubmit} className="p-4 sm:p-6 space-y-3 text-xs">
              {clientModalError && (
                <div className="p-2.5 bg-rose-50 border border-rose-300 text-rose-800 rounded-lg text-xs font-semibold">
                  ⚠️ {clientModalError}
                </div>
              )}
              <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">DNI / CUIT:</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Domicilio:</label>
                  <input
                    type="text"
